...
import CancellationMailJob from '../jobs/cancellation-mail.job';
...
class AppointmentController {
    ...
    async store(req, res) {
        ...
        if (req.userId === provider_id) {
            return res
                .status(401)
                .json('You can only create appointments for other providers');
        }
    ...
    async delete(req, res) {
        ...
        // verifica se está com duas horas de antecedência
        if (isBefore(dateWithSub, new Date())) {
            return res.status(401).json({
                error: 'You con only cancel appointments 2 hours in advance',
            });
        }
